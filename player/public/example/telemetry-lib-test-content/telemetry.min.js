...
!function(s) {
    ...
    this.startTime = 0, this._defaultValue = {
        ...
        }, isActive = !0, Telemetry.config = config, console.log("Telemetry config ", Telemetry.config);
        ...
        if (!this.hasRequiredData(data, [ "pageid", "type" ])) return void console.error("Invalid impression data");
        var eksData = {
            stageid: pageid,
        ...
        if (!this.hasRequiredData(data, [ "type", "id" ])) return void console.error("Invalid interact data");
        var eksData = {
        ...
        return Telemetry.isActive ? Telemetry.flushEvent(Telemetry.instance.assessEnd(assessStartEvent, data)) : new InActiveEvent();
    }, Telemetry.response = function(data) {
        if (!this.hasRequiredData(data, [ "target", "qid", "type" ])) return void console.error("Invalid response data");
        var eksData = {
            target: data.target,
            qid: data.qid,
            type: data.type,
        ...
        if (!this.hasRequiredData(data, [ "type" ])) return void console.error("Invalid interrupt data");
        var eksData = {
            type: data.type,
            stageid: data.pageid || ""
        };
        ...
        if (!this.hasRequiredData(data, [ "err", "errtype" ])) return void console.error("Invalid error data");
        var eksData = {
            err: data.err,
            type: data.errtype,
            env: data.env || "",
        ...
        return Telemetry.isActive ? this.flushEvent(Telemetry.instance.end(data)) : new InActiveEvent();
    }, Telemetry.exdata = function(type, data) {
        return Telemetry.isActive ? this.flushEvent(Telemetry.instance.xapi(type, data)) : new InActiveEvent();
