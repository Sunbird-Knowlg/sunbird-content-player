...
!function(s) {
    ...
    return this.Telemetry = function() {}, Telemetry.initialized = !0, Telemetry.config = void 0, 
    Telemetry._version = "2.2", this.startTime = 0, this._defaultValue = {
        pdata: {
            id: "genie",
            "var": "6.5.2567",
            pid: ""
        ...
        var requiredData = Object.assign(config, {
            contentId: contentId,
            contentVer: contentVer,
            type: type
        });
        if (!hasRequiredData(requiredData, [ "contentId", "contentVer", "pdata", "channel", "uid", "authtoken" ])) return console.error("Invalid start data"), 
        void (Telemetry.initialized = !1);
        init(config, contentId, contentVer);
        var startEventObj = getEvent("OE_START", data);
        addEvent(startEventObj), startTime = startEventObj.ets;
    }, Telemetry.impression = function(pageid, type, subtype, data) {
        if (void 0 == pageid || void 0 == type) return void console.error("Invalid impression data");
        var eksData = {
            stageid: pageid,
        ...
        if (!hasRequiredData(data, [ "type", "id" ])) return void console.error("Invalid interact data");
        var eksData = {
        ...
        if (void 0 == qid) return void console.error("Invalid interact data");
        var eksData = {
            qid: qid,
        ...
        if (!hasRequiredData(data, [ "qtitle", "qdesc", "mmc", "mc" ])) return void console.error("Invalid end assessment data");
        if (void 0 == assessStartEvent) return void console.error("Invalid end assessment data");
        ...
        var endeks = Object.assign(assessStartEvent.edata.eks, {
            ...
            mmc: data.mmc,
            mc: data.mc,
            length: Math.round((new Date().getTime() - assessStartEvent.ets) / 1e3)
        });
        addEvent(getEvent("OE_ASSESS", endeks));
    }, Telemetry.response = function(data) {
        if (!hasRequiredData(data, [ "target", "qid", "type" ])) return void console.error("Invalid response data");
        var eksData = {
            target: data.target,
            qid: data.qid,
            type: data.type,
        ...
        if (!hasRequiredData(data, [ "type" ])) return void console.error("Invalid interrupt data");
        var eksData = {
            type: data.type,
            stageid: data.pageid || ""
        };
        ...
        if (!hasRequiredData(data, [ "err", "errtype" ])) return void console.error("Invalid error data");
        var eksData = {
            err: data.err,
            type: data.errtype,
            env: data.env || "",
        ...
        var eksData = {
            ...
            stageid: data.pageid || "",
            length: (new Date().getTime() - startTime) / 1e3
        };
        ...
        _defaultValue.gdata = {
            id: contentId,
            ver: contentVer
        }, config.batchsize = config.batchsize ? config.batchsize < 10 ? 10 : config.batchsize > 1e3 ? 1e3 : config.batchsize : _defaultValue.batchsize, 
        Telemetry.config = Object.assign(_defaultValue, config), console.log("Telemetry config ", Telemetry.config);
        ...
        var isValid = !0;
        return mandatoryFields.forEach(function(key) {
            data.hasOwnProperty(key) || (isValid = !1);
        }), isValid;
    ...
    }, "function" != typeof Object.assign && objectAssign(), Telemetry;
}();
...
var TelemetrySyncManager = {
    ...
    sendTelemetry: function(event) {
        var telemetryEvent = event.detail, instance = TelemetrySyncManager;
        if (instance._teleData.push(telemetryEvent), "OE_END" == telemetryEvent.eid.toUpperCase() || instance._teleData.length >= Telemetry.config.batchsize) {
            var telemetryData = instance._teleData, telemetryObj = {
                id: "ekstep.telemetry",
                ver: Telemetry._version,
                ets: new Date().getTime(),
                events: telemetryData
            }, headersParam = {};
            "undefined" != typeof Telemetry.config.authtoken && (headersParam.Authorization = "Bearer " + Telemetry.config.authtoken);
            ...
            jQuery.ajax({
                ...
                instance._teleData = [], console.log("Telemetry API success", resp);
