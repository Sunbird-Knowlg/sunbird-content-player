...
app.compileProvider.directive('mute', function($rootScope) {
    return {
        restrict: 'E',
        template: '<div role="button" tabindex="0" aria-label="Turn off Sound" ng-keydown="$event.keyCode === 13 && toggleMute()" ng-click="toggleMute()"><img ng-src="{{muteImg}}"/><span> Turn {{AppLables.mute}} Sound </span></div>',
...
app.compileProvider.directive('stageInstructions', function($rootScope) {
    return {
        restrict: 'E',
        template: '<div ng-class="{\'icon-opacity\' : !stageData.params.instructions}" role="button" tabindex="0" aria-disabled="true" ng-keydown="$event.keyCode === 13 && showInstructions()" ng-click="showInstructions()"><img ng-src="{{imageBasePath}}icn_teacher.png" style="z-index:2;" alt="note img"/><span> {{AppLables.instructions}} </span></div>',
...
app.compileProvider.directive('goodJob', function($rootScope) {
    return {
        restrict: 'E',
        template: '<div class="popup"> <div class="popup-overlay" ng-click="hidePopup()"></div> <div class="popup-full-body"> <div class="font-lato assess-popup assess-goodjob-popup"> <div class="correct-answer" style=" text-align: center;"> <div class="banner"> <img ng-src="{{imageBasePath}}banner3.png" height="100%" width="100%"> </div> <div class="sign-board"> <img ng-src="{{imageBasePath}}check.png" id="correctButton" width="40%" /> </div> </div> <div class="popup-buttons-container"> <div ng-click="hidePopup(); moveToNextStage(\'next\');" ng-keydown="$event.keyCode === 13 && hidePopup(); moveToNextStage(\'next\');" tabindex="0" aria-label="{{AppLables.next}}" role="button" class="primary center button">{{AppLables.next}}</div> </div> </div> </div> </div>',
        controller: function($scope, $rootScope, $timeout) {
...
app.compileProvider.directive('tryAgain', function($rootScope) {
    return {
        restrict: 'E',
        template: '<div class="popup"> <div class="popup-overlay" ng-click="hidePopup()"></div> <div class="popup-full-body"> <div class="font-lato assess-popup assess-tryagain-popup"> <div class="wrong-answer" style=" text-align: center;"> <div class="banner"> <img ng-src="{{imageBasePath}}banner2.png" height="100%" width="100%"> </div> <div class="sign-board"><img ng-src="{{imageBasePath}}incorrect.png" width="40%" alt="Incorrect Icon" id="incorrectButton" /> </div> </div> <div class="popup-buttons-container"> <div ng-click="hidePopup(); moveToNextStage(\'next\');" ng-keydown="$event.keyCode === 13 && hidePopup(); moveToNextStage(\'next\');" tabindex="0" aria-label="{{AppLables.next}}" role="button" class="left button">{{AppLables.next}}</div> <div ng-click="retryAssessment(\'gc_retry\', $event);" ng-keydown="$event.keyCode === 13 && retryAssessment(\'gc_retry\', $event);" href="javascript:void(0);" tabindex="0" aria-label="{{AppLables.tryAgain}}" role="button" class="right primary button">{{AppLables.tryAgain}}</div> </div> </div> </div> </div>'
    }
});
