...
ControllerManager.registerController("items", ItemController), MCQEvaluator = {
    ...
    moveToEndPage: function() {
        if (config.showEndPage) {
            ...
            Renderer.theme.setParam(stage.getStagestateKey(), stage._currentState), EkstepRendererAPI.dispatchEvent("renderer:init:endpage"), 
            window.location.hash = "/content/end/" + GlobalContext.currentContentId, AudioManager.stopAll();
...
PluginManager.registerPlugin("input", InputPlugin), org.ekstep.contentrenderer.registerPlguin("launcher", Plugin.extend({
    initialize: function() {
    ...
    initLauncher: function(evt, content) {
        ...
        isCoreplugin = !0, org.ekstep.contentrenderer.initPlugins("renderer"), _.isUndefined(contentTypePlugin) || (this.loadPlugin(contentTypePlugin, content), 
        isCoreplugin = !1);
    },
    loadPlugin: function(plugin, content) {
        org.ekstep.contentrenderer.loadPlugins(plugin, [], function() {
            EkstepRendererAPI.dispatchEvent("content:load:" + content.mimeType, void 0, content);
        });
    }
}));
