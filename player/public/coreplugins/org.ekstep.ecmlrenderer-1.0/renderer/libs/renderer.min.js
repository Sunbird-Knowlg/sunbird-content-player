...
var ItemController = Controller.extend({
    ...
    evalItem: function() {
        try {
            ...
            result && (pass = result.pass, item.score = result.score), !_.isUndefined(item.concepts)) var concepts = _.isArray(item.concepts) && _.isObject(item.concepts) ? item.concepts : JSON.parse(item.concepts);
            var data = {
                pass: result.pass,
                score: item.score,
                res: result.res,
...
ControllerManager.registerController("items", ItemController), MCQEvaluator = {
    ...
    handleSubmit: function() {
        ...
        if (!_.isUndefined(Renderer.theme) && _.isUndefined(Renderer.theme.getParam(eventName)) && _.isUndefined(Renderer.theme._currentScene.getParam(eventName)) && (val = AppConfig.overlay.showSubmit ? "on" : "off"), 
    ...
    moveToEndPage: function() {
        if (config.showEndPage) {
            ...
            Renderer.theme.setParam(stage.getStagestateKey(), stage._currentState), Renderer.theme._currentStage = void 0, 
            EkstepRendererAPI.showEndPage(), AudioManager.stopAll();
...
var TelemetryPlugin = Plugin.extend({
    ...
    initializeTelemetryPlugin: function() {
        if ("undefined" == typeof cordova) {
            ...
            this._requiredFields.uid = extConfig.context.uid || AppConfig.uid, this._requiredFields.sid = extConfig.context.sid || CryptoJS.MD5(Math.random().toString()).toString(), 
    ...
    sendTelemetry: function(telemetryData) {
        ...
        }, configuration = EkstepRendererAPI.getGlobalConfig(), headers = {};
...
var ThemePlugin = Plugin.extend({
    ...
    render: function() {
        ...
        }), this.invokeStage(this._data.startStage)), this.update(), EkstepRendererAPI.dispatchEvent("renderer:content:start");
    },
    addController: function(p) {
