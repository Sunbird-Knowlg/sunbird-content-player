...
ControllerManager.registerController("items", ItemController), MCQEvaluator = {
    ...
    evaluate: function(item) {
        ...
        if (item) {
            ...
            if (_.isArray(options) && _.each(options, function(opt) {
                ...
                pass = _.isEqual(_.pluck(options, "selected"), _.pluck(options, "answer"));
            }
            pass || (result.feedback = item.feedback, item.partial_scoring || (score = 0));
        }
        return result.pass = pass, result.score = score, result.res = res, result;
    ...
    handleSubmit: function() {
        ...
        if (!_.isUndefined(Renderer.theme) && _.isUndefined(Renderer.theme.getParam(eventName)) && _.isUndefined(Renderer.theme._currentScene.getParam(eventName)) && (val = AppConfig.overlay.overlaySubmit), 
        _.isUndefined(Renderer.theme) || _.isUndefined(Renderer.theme._currentScene) || !Renderer.theme._currentScene.isItemScene()) EventBus.dispatch(eventName, "off"); else {
            if ("on" == val) {
                val = Renderer.theme._currentScene.isReadyToEvaluate() === !0 ? "enable" : "disable";
            }
            EventBus.dispatch(eventName, val), this.handleEcmlElements(eventName, val);
        }
    },
...
var StopCommand = Command.extend({
    ...
    invoke: function(action) {
        var plugin = this.getPluginObject();
        void 0 === plugin && (plugin = AudioManager), action.sound === !0 ? AudioManager[this._stopAllMethod](action) : plugin[this._stopMethod](action);
    }
});
...
var ThemePlugin = Plugin.extend({
    ...
    render: function() {
        ...
        }), this.invokeStage(this._data.startStage)), this.update(), EkstepRendererAPI.dispatchEvent("renderer:splash:hide"), 
...
android.recorder = {
    ...
    processRecording: function(lineindex, filePath, cb) {
        (filePath = filePath ? filePath : speech.mediaInstance ? speech.mediaInstance.filePath : "") && lineindex && speech.getRecorder().process(filePath, lineindex).then(function(response) {
