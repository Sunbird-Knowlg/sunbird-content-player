...
PluginManager = {
    ...
    isController: function(type) {
        return ControllerManager.controllerMap[type] ? !0 : !1;
    },
...
var Controller = Class.extend({
    ...
    getModel: function() {
        ...
        if (_.isArray(this._model)) {
            ...
            0 > index && (index = 0), m = this._model[index];
        } else m = this._model;
        return m;
    },
    ...
    hasNext: function() {
        return this._loaded && this._index < this._repeat - 1 ? !0 : !1;
    },
    hasPrevious: function() {
        return this._loaded && this._index > 0 ? !0 : !1;
    },
    next: function() {
        var d;
...
var ItemController = Controller.extend({
        ...
        try {
            if (this.hasNext()) {
                ...
                if (item) {
                    ...
                    d = item.model, item.qid = _.isString(item.qid) && !_.isEmpty(item.qid) ? item.qid : "na";
                    try {
    ...
    evalItem: function() {
        try {
            var result, instance = this, item = this.getModel(), pass = !1;
            if (console.log("evalItem: ", item), GlobalContext.registerEval[item.type.toLowerCase()]) {
                var customEvalInstance = GlobalContext.registerEval[item.type.toLowerCase()];
                result = customEvalInstance.evaluate(item);
            } else "ftb" == item.type.toLowerCase() ? result = FTBEvaluator.evaluate(item) : "mcq" == item.type.toLowerCase() || "mmcq" == item.type.toLowerCase() ? result = MCQEvaluator.evaluate(item) : "mtf" == item.type.toLowerCase() && (result = MTFEvaluator.evaluate(item));
            if (result && (pass = result.pass, item.score = result.score), !_.isUndefined(item.concepts)) var concepts = _.isArray(item.concepts) && _.isObject(item.concepts) ? item.concepts : JSON.parse(item.concepts);
            var data = {
                pass: result.pass,
                score: item.score,
                res: result.res,
    ...
    feedback: function() {
        ...
        if (feedback) {
            ...
            feedback.forEach(function(range) {
                ...
                percent >= min && max >= percent && (message = range.message);
            });
        }
        return message;
    }
...
ControllerManager.registerController("items", ItemController), MCQEvaluator = {
    ...
    _isCorrectAnswer: function(actual, given) {
        ...
        } else isCorrect = actual == given ? !0 : !1; else isCorrect = actual == given ? !0 : !1;
        return isCorrect;
    },
    reset: function(item) {
        if (item) {
    ...
    showFeeback: function(showOverlayGoodJob) {
        var returnVal = !0;
        return showOverlayGoodJob ? (returnVal = "off" == this._stageConfig.overlayGoodJob ? !1 : !0, 
        this.showGoodJobFb(returnVal)) : (returnVal = "off" == this._stageConfig.overlayTryAgain ? !1 : !0, 
    ...
    navigatePrevious: function() {
        try {
            ...
            if (_.isUndefined(navigateToStage) && (!Renderer.theme._currentScene.isItemScene() || !Renderer.theme._currentScene._stageController.hasPrevious())) {
                var rendererData = EkstepRendererAPI.getContentData(), currentStage = getCurrentStageId();
                return void (rendererData.startStage === currentStage && contentExitCall());
            }
    ...
    isStageAssetsLoaded: function(stageId) {
        ...
        return !_.isUndefined(this.loaders[stageId]) && this.loaders[stageId].stageLoaded ? !0 : _.isUndefined(this.loaders[stageId]) ? !0 : !1;
    }
}), RecorderManager = {
...
var DataGenerator = {
    ...
    _addItems: function(id, count, items, list, shuffle, optionShuffle, cumulativeIndex) {
        ...
        if (_.isArray(set)) {
            ...
            for (var pick = [], qindex = 0, i = 0; count > i; i++) {
                if (shuffle) {
    ...
    _callBack: function(response) {
        _(Renderer.theme).isUndefined() || "undefined" != typeof response && "success" == response.status && this._invokeRelatedActions("children");
    }
...
var LayoutPlugin = Plugin.extend({
    ...
    initPlugin: function(data) {
        ...
        data.strokeWidth && (data["stroke-width"] = data.strokeWidth), data["stroke-width"] && graphics.setStrokeStyle(data["stroke-width"]);
        ...
        _.isEmpty(asset)) this._render = !1, console.warn("ImagePlugin: Asset not found", data); else {
            ...
            if (_.isString(assetSrc)) this._self.visible = !1, AssetManager.strategy.loadAsset(this._stage._data.id, asset, assetSrc, function() {
                Renderer.update = !0, setTimeout(function() {
                    ...
                    instance._self.visible = _.isUndefined(instance._data.visible) ? !0 : instance._data.visible, 
                    Renderer.update = !0;
                }, 100);
            }); else {
                var sb = s.getBounds();
...
var PlaceHolderPlugin = Plugin.extend({
    ...
    renderGridLayout: function(parent, instance, data) {
        var assetId = instance.param.asset, assetSrc = instance._theme.getAsset(assetId), img = new createjs.Bitmap(assetSrc), getImage = function(cb) {
            ...
            var x = 0, y = 0, pad = (instance.dimensions().w * instance.dimensions().h, instance.dimensions().pad || 0), n = instance.param.count, rectHeight = instance.dimensions().h, rectWidth = instance.dimensions().w, imgCountRow = Math.ceil(Math.sqrt(n * rectHeight / rectWidth));
            ...
            var pixelPerImg = pixelPerImgX > pixelPerImgY ? pixelPerImgX : pixelPerImgY, param = instance.param, paddedImg = instance.getAssetBound(img, pad), assetBounds = paddedImg.getBounds(), assetW = assetBounds.width, assetH = assetBounds.height;
...
var StagePlugin = Plugin.extend({
    ...
    evaluate: function(action) {
        var isEvaluated = _.isUndefined(this._currentState) ? !1 : this._currentState.isEvaluated;
    ...
    stateConfig: function() {
        return _.isUndefined(this._stageController) ? !0 : void 0 == this._stageController._data.saveState || 1 == this._stageController._data.saveState ? !0 : !1;
    },
    getParam: function(param) {
    ...
    isItemScene: function() {
        ...
        return _.isUndefined(stageCtrl) || _.isUndefined(stageCtrl._model) || "items" != stageCtrl._type ? !1 : !0;
    },
        ...
        try {
            ...
            if (_.isUndefined(startStage)) {
                var firstStage = _.find(this._data.stage, function(stage) {
                    return stage.param && _.isUndefined(firstStage) ? stage : void 0;
                });
...
var ScribblePlugin = Plugin.extend({
    ...
    show: function() {
        this._self.visible = this._self.parent.visible = !0, Renderer.update = !0;
    },
    hide: function() {
        this._self.visible = this._self.parent.visible = !1, Renderer.update = !0;
    },
    toggleShow: function() {
        this._self.visible = this._self.parent.visible = !this._self.visible, Renderer.update = !0;
    },
    drawBorder: function() {}
});
...
var VideoPlugin = Plugin.extend({
    ...
    initPlugin: function(data) {
        this._data = data, this._data.muted = AudioManager.muted ? !0 : !1, this._data && (_.isUndefined(this._data.autoplay) && (this._data.autoplay = !0), 
        _.isUndefined(this._data.controls) && (this._data.controls = !1)), _instance = this, 
        _instance.loadVideo(data);
    },
    loadVideo: function(data) {
        if (!data.asset) return !1;
        var lItem = this.createVideoElement(), videoEle = this.getVideo();
