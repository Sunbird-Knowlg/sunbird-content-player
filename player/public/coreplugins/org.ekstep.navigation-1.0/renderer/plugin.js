 ...
 */
 ...
 Plugin.extend({
  ...
  initialize: function() {
    ...
    this.custom_top_next = org.ekstep.pluginframework.pluginManager.resolvePluginResource(this._manifest.id, this._manifest.ver, "renderer/controller/custom-top-next.html");
    this.custom_top_previous = org.ekstep.pluginframework.pluginManager.resolvePluginResource(this._manifest.id, this._manifest.ver, "renderer/controller/custop-top-previos.html");
    this.default_middle_next = org.ekstep.pluginframework.pluginManager.resolvePluginResource(this._manifest.id, this._manifest.ver, "renderer/controller/default-middle-next.html");
    this.default_middle_previous = org.ekstep.pluginframework.pluginManager.resolvePluginResource(this._manifest.id, this._manifest.ver, "renderer/controller/default-middle-previous.html");
    ...
    EkstepRendererAPI.addEventListener("renderer:navigation:prev",function(event){
      ...
      setTimeout(function(){ 
        var pluginInstance = instance._customNavigationPlugins[0];
        if( pluginInstance && pluginInstance._itemIndex > 0){
            EventBus.dispatch("renderer:previous:enable");
          }
      }, 500);
    });
