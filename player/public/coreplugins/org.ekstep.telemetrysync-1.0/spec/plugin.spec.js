describe('TelemetrySync Plugin', function() {
    ...
    describe("When initializeTelemetryPlugin is invoked", function() {
        it("It should invoke listenTelementryEvent", function() {
            var someObject = jasmine.createSpyObj('someObject', [ 'detectClient' ]);
            someObject.detectClient.and.callFake(function() {
                throw 'an-exception';
            });

            expect(EkstepRendererAPI).not.toBeUndefined();
            spyOn(TelemetrySync, "listenTelementryEvent").and.callThrough();
            spyOn(TelemetrySync, "initializeTelemetryPlugin").and.callThrough();
            ...
            expect(TelemetrySync.initializeTelemetryPlugin).toHaveBeenCalled();
        ...
        it("It should merge did, uid, sid with telemetry event", function() {
            EventBus.addEventListener("telemetryEvent", function(data) {
                ...
                if (parseInt(data.ver) >= 3) {
                    ...
                    var newEvent = TelemetrySync.appendRequiredFields(data);
