...
Plugin.extend({
    initialize: function() {
        app.compileProvider.directive("genie", ["$rootScope", function($rootScope) {
            return {
                restrict: "E",
                template: '<div ng-class="enableGenie ? \'genie-home\' : \'icon-opacity genie-home\'" role="button" aria-label="{{AppLables.exit || \"Exit\"}}" tabindex="0" aria-disabled="true" ng-click="goToGenie()"><img ng-src="{{imageBasePath}}icn_home.png"/><span> {{AppLables.exit || "Exit"}}</span></div>',link: function(scope) {
