...
angular.module('genie-canvas', ['ionic', 'ngCordova', 'genie-canvas.services'])
    ...
    .run(function($rootScope, $ionicPlatform, $location, $timeout, $state, $stateParams, appConstants, ContentService) {
        ...
        $scope.getContentData = function(content) {
            $rootScope.showPage = true;
            if (GlobalContext.config.appInfo && GlobalContext.config.appInfo.identifier) {
                if ((isbrowserpreview == "true")) {
                    if (content.metadata && (content.metadata.mimeType != COLLECTION_MIMETYPE)) {
                        ...
                        //For collections
                        $scope.getContentMetadata(content.identifier);
                    }
                } else {
                    //For mobile
                    $scope.getContentMetadata(content.identifier);
        ...
        $rootScope.getContentMetadata = function(content,cb) {
            jQuery('#loading').hide();
        ...
        $rootScope.setContentMetadata = function(contentData,cb) {
        ...
        $scope.init = function() {
            if (_.isUndefined($rootScope.content)) {
                if (_.isEmpty(content)) {
                    $scope.getContentMetadata(GlobalContext.game.id, function() {
                        $scope.renderContent();
                    });
                    $rootScope.content = content.metadata;
                } else {
                    $rootScope.content = content.metadata;
                    $rootScope.setContentMetadata($rootScope.content, function() {
                        $scope.renderContent();
                    });
                }
            } else {
        ...
        $scope.renderContent = function(){
            if ($stateParams.itemId && $rootScope.content) {
                var identifier = ($rootScope.content && $rootScope.content.identifier) ? $rootScope.content.identifier : null;
                var pkgVersion = !_.isUndefined($rootScope.content.pkgVersion) ? $rootScope.content.pkgVersion.toString() : null;
                var version = ($rootScope.content && pkgVersion) ? pkgVersion : "1";
                startTelemetry(identifier, version);
        ...
        /*$scope.$on('loadRenderer', function(event) {
            $scope.init();
        });*/
        setTimeout(function() {
            $scope.init();
