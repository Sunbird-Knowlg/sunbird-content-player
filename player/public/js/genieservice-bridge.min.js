...
dateFormat.masks = {
    ...
    init: function(gameData, user) {
        ...
        return _.isEmpty(localStorageInstance) || TelemetryService.setTelemetryService(localStorageInstance, gameData), 
        new Promise(function(resolve, reject) {
    ...
    setTelemetryService: function(localStorageInstance, gameData) {
        if (localStorageInstance._gameData.id == gameData.id) {
            ...
            if (!_.isUndefined(end)) {
                var teEndevent = TelemetryService.instance.createEvent("OE_END", {}).start(), startTime = 0;
                end.length > 0 && (startTime = end[end.length - 1].startTime), teEndevent.startTime = startTime, 
    ...
    getLocalStorageInstance: function() {
        ...
        return _.isEmpty(telemetryLocalStorageData) || (telemetryLocalStorageData._start = JSON.parse(telemetryLocalStorageData._start), 
        telemetryLocalStorageData._end = JSON.parse(telemetryLocalStorageData._end)), telemetryLocalStorageData;
    },
    ...
    levelSet: function(eventData) {
        if (TelemetryService.isActive) return new InActiveEvent();
    },
    ...
    navigate: function(stageid, stageto) {
        return TelemetryService.isActive ? (console.log(""), "1.0" == TelemetryService._version ? "" : this.flushEvent(TelemetryService.instance.navigate(stageid, stageto), TelemetryService.apis.telemetry)) : new InActiveEvent();
    },
    ...
    exit: function() {
        if (TelemetryService.isActive) {
            if (TelemetryService._data = [], !_.isEmpty(TelemetryService.instance._end)) for (var len = TelemetryService.instance._end.length, i = 0; i < len; i++) TelemetryService.end();
    ...
    assessEnd: function(eventObj, data) {
        if (eventObj) return eventObj._isStarted || (eventObj._isStarted = !0), eventObj.event.edata.eks.score = data.score || 0, 
    ...
    assessEnd: function(eventObj, data) {
        ...
        }) : eventObj.event.edata.eks.resvalues = [], eventObj.end(), eventObj;
    },
    interrupt: function(type, id) {
