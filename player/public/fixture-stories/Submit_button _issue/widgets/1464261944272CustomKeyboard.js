Plugin.extend({
    ...
    initPlugin: function(data) {
		...
		if(data.type=="numeric")
		{
			...
			{
				if(i == 0){
					customKeys += '<div class="key_numeric firstkey" id="'+buttons[i]+'_btn">'+buttons[i]+'</div>';
				}else{
					customKeys += '<div class="key_numeric" id="'+buttons[i]+'_btn">'+buttons[i]+'</div>';
				}
				//buttonSounds[i] = new Audio('assets/'+buttons[i]+'.mp3');
			}
			var clearKey = '<div class="key_numeric key_numeric_clear" id="clear_btn"></div>';
			customKeys += clearKey;
		...
		}else if(data.type=="custom")
		{
			...
			{
				var difference = 26-buttons.length;
