...
genieservice_web = {
    ...
    getCurrentUser: function() {
        return new Promise(function(resolve, reject) {
            if (genieservice_html.localData.user) {
                resolve(_.isArray(genieservice_html.localData.user) ? genieservice_html.localData.user[0] : genieservice_html.localData.user);
    ...
    getLocalData: function(callback) {
        ...
        genieservice_html._jsFilesToLoad.push("test/fixture-content-list.json"), genieservice_html._jsFilesToLoad.push("test/fixture-user-list.json"), 
        genieservice_html._jsFilesToLoad.push("test/fixture-word-list.json"), genieservice_html.loadJsFilesSequentially();
    },
    loadJsFilesSequentially: function() {
        if (genieservice_html._jsFilesToLoad[genieservice_html._jsFileIndex]) {
            ...
            jQuery.getJSON(fileLoaded, function(json) {
                _.extend(genieservice_html.localData, json), console.log("LocalData json loaded", genieservice_html.localData), 
                genieservice_html._jsFileIndex = genieservice_html._jsFileIndex + 1, genieservice_html.loadJsFilesSequentially();
            });
...
var genieServiceBridge = function() {
    function initialize() {
        ...
        if (isMobile) loadCordova(); else if ("undefined" == typeof AppConfig && "undefined" == typeof isbrowserpreview) {
            var flavor = getUrlParameter("flavor"), launchData = getUrlParameter("launchData");
            if (flavor) {
                var flavors = [ "dev", "qa", "production" ];
                _.contains(flavors, flavor.toLowerCase()) && (AppConfig = {
                    flavor: flavor
                }, launchData && (launchData = JSON.parse(launchData), AppConfig[flavor] = launchData.envpath), 
                genieservice = genieservice_portal, _callbackFunc());
            } else genieservice.getLocalData(function() {
                _callbackFunc();
            });
        }
    }
