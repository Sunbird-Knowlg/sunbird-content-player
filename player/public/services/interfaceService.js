org.ekstep.service.content = new (org.ekstep.service.mainService.extend({
	...
	_prepareContent: function (item) {
		...
		if (item.path && data) {
			...
			path = (isbrowserpreview == false) ? "file://" + path : path
			data.baseDir = path
			globalConfig.basepath = path
			if (isbrowserpreview === false) { data.appIcon = (data.appIcon) ? path + "/" + data.appIcon : path + "/logo.png" } else { data.appIcon = (isbrowserpreview) ? data.appIcon : path + "/logo.png" }
