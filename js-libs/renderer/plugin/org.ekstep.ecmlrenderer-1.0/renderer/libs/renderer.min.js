...
PluginManager.registerPlugin("input", InputPlugin), PluginManager.registerPlugin("launcher", Plugin.extend({
    ...
    initLauncher: function(evt, content) {
        ...
        isCoreplugin = !0, PluginManager.init("renderer"), _.isUndefined(contentTypePlugin) || (this.loadPlugin(contentTypePlugin, content), 
        isCoreplugin = !1);
    },
...
PluginManager.registerPlugin("video", VideoPlugin), PluginManager.registerPlugin("endpage", Plugin.extend({
    myApp: void 0,
    initialize: function() {
        EkstepRendererAPI.addEventListener("renderer:init:endpage", this.loadControllers, this);
    },
    loadControllers: function() {
        angular.element(document.body).scope().$root.content.name = "Test Content Name", 
        app.controllerProvider.register("EndPageCtrl", function($scope, $rootScope, $state, ContentService, $stateParams) {
            ...
            $scope.arrayToString = function(array) {
                ...
                _.isNull(muteElement) || (muteElement.style.display = "none"), AudioManager.unmute();
            }, $scope.showFeedback = function(param) {
                $scope.userRating = param, $scope.popUserRating = param, $scope.showFeedbackPopup = !0, 
                ...
                $rootScope.content = content), localStorageGC.setItem("content_old", $rootScope.content), 
                _(TelemetryService.instance).isUndefined()) {
                    var tsObj = localStorageGC.getItem("telemetryService"), correlationData = [];
                ...
                TelemetryService.interact("TOUCH", $stateParams.contentId, "TOUCH", {
                    ...
                    subtype: "ContentID"
                }), $scope.$broadcast("getRelatedContentEvent"), angular.element(jQuery("popup[id='creditsPopup']")).trigger("popupUpdate", {
    ...
    loadDirectives: function() {
        app.compileProvider.directive("genie", function($rootScope) {
            return {
                ...
                link: function(scope) {
                    scope.languageSupport = $rootScope.languageSupport, scope.enableGenie = "undefined" != typeof cordova, 
                    scope.imgSrc = $rootScope.imageBasePath + scope.icon, scope.enableGenie && (scope.goToGenie = function() {
            ...
            return {
                ...
                link: function(scope) {
                    scope.restartContent = function() {
                        ...
                        var muteElement = document.getElementById("unmute_id");
                        _.isNull(muteElement) || (muteElement.style.display = "none"), AudioManager.unmute(), 
                        !_.isUndefined(scope.hideMenu) && scope.menuOpened && scope.hideMenu();
                    };
                }
            };
