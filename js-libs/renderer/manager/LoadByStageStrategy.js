    ...
    getAsset: function(stageId, assetId) {
        ...
        if (!asset) {
            if (this.assetMap[assetId]) {
                ...
                EkstepRendererAPI.getTelemetryService().error({'message':'Asset not found. Please check index.ecml'},{'type':'content','severity':'error','action':'play', 'asset':assetId, 'objectid':assetId})
        };
        return asset;
    },
